...
# ... (dentro Quadrante 2) ...
    ...
    with st.container(border=True):
       ...
       # ... dentro QUADRANTE 3 ...
        ...
        memory_limit = st.number_input("⚡ Context Window (Msg)", min_value=0, max_value=100, value=1, help="Numero di messaggi passati da ricordare")
...
# ==========================================
    ...
    # ==========================================
        ...
        try:
            ...
            # --- RAMO B: CHAT STANDARD (CORRETTO) ---
            else:
                model = genai.GenerativeModel(model_id)
                
                # 1. RECUPERO MESSAGGI (Comune a entrambi)
                msgs_raw = get_context_with_pins(st.session_state.messages[:-1], memory_limit)
                ...
                # 2. DECISIONE
                # Semplifichiamo: se l'utente vuole Smart RAG, lo facciamo (se ci sono messaggi).
                do_compression = st.session_state.use_smart_rag and len(msgs_raw) > 0

                # --- CASO 1: COMPRESSIONE ATTIVA (Metodo Lite) ---
                if do_compression:
                    with st.status("⚡ Compressione Contesto (Flash-Lite)...", expanded=False) as status:
                        try:
                            ...
                            # IMPORTANTE: In modalità compressione, la history formale per Google deve essere vuota
                            # perché il contesto è stato iniettato dentro 'final_prompt_to_send'.
                            final_history = [] 
                            
                            status.update(label="✅ Compressione riuscita!", state="complete")
                        
                        except Exception as e:
                            # Fallback emergenza: Se fallisce, usiamo il raw
                            status.update(label="⚠️ Errore Compressione - Uso Raw", state="error")
                            final_history = []
                            for m in msgs_raw:
                                role_google = "user" if m["role"] == "user" else "model"
                                final_history.append({"role": role_google, "parts": [str(m["content"])]})
                            final_prompt_to_send = f"[SYSTEM: File raw]\n{file_summary_text}\n\nDOMANDA: {prompt}"

                # --- CASO 2: RAW MODE (Fedeltà Massima) ---
                else:
                    final_history = []
                    for m in msgs_raw:
                        role_google = "user" if m["role"] == "user" else "model"
                        content_clean = str(m["content"])
                        final_history.append({"role": role_google, "parts": [content_clean]})
                    
                    if file_summary_text:
                        final_prompt_to_send = f"[DATI FILE CARICATI]:\n{file_summary_text}\n\n[MESSAGGIO UTENTE]:\n{prompt}"
                    else:
